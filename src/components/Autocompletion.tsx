...
export const Autocompletion: React.FC = () => {
  ...
  const acceptSuggestion = (event: KeyboardEvent) => {
    if (event.key === 'Tab' && currentSuggestion && activeTextArea) {
      ...
      setCurrentSuggestion('')
    } else if (event.key === 'Esc' && currentSuggestion && activeTextArea) {
      rejectSuggestion(event)
    }
  }

  const rejectSuggestion = (event: KeyboardEvent) => {
    if (event.key === 'Esc' && currentSuggestion && activeTextArea) {
      event.preventDefault()
